class LabeledFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def collection_check_boxes(attribute, records, record_id, record_name)
    content_tag :div, :class => "element-wrapper clearfix" do
      @template.hidden_field_tag("#{object_name}[#{attribute}][]") + records.map do |record|
  ...
  def submit(*args)
    content_tag :div, :class => "button-wrapper submit-button-wrapper clearfix" do
      super
    end
  end
  ...
  def error_messages
    if object.errors.full_messages.any?
      content_tag(:div, :class => "error_messages") do
        content_tag(:h2, "Invalid Fields") + content_tag(:ul) do
